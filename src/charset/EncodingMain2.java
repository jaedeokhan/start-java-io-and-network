...
public class EncodingMain2 {
    ...
    public static void main(String[] args) {
        ...
        test("가", UTF_8, UTF_8);            // O, 3byte 사용
        test("가", UTF_16BE, UTF_16BE);      // O

        System.out.println("== 한글 인코딩 - 복잡한 문자 ==");
        test("뷁", EUC_KR, EUC_KR);          // X
        test("뷁", MS_949, MS_949);          // O, EUC_KR을 확장해서 모든 한글 가능
        test("뷁", UTF_8, UTF_8);            // O
        test("뷁", UTF_16BE, UTF_16BE);      // O

        System.out.println("== 한글 인코딩 - 디코딩이 다른 경우 ==");
        test("가", EUC_KR, MS_949);
        test("뷁", MS_949, EUC_KR);          // 인코딩은 가능, 디코딩 X (EUC_KR은 뷁 X)
        test("가", EUC_KR, UTF_8);           // 인코딩도 byte 다름, 디코딩 X (UTF_8은 3byte 처리로 다름)
        test("가", UTF_8, EUC_KR);           // 인코딩으 byte 다름, 디코딩 X (EUC_KR은 2byte 처리로 다름)
        test("가", MS_949, UTF_8);           // X
        test("가", UTF_8, MS_949);           // X

        System.out.println("== 영문 인코딩 - 디코딩이 다른 경우 ==");
        test("A", EUC_KR, UTF_8);           // EUC_KR, UTF_8 모두 ASCII 지원
        test("A", MS_949, UTF_8);           // O
        test("A", UTF_8, MS_949);           // O
        test("A", UTF_8, UTF_16BE);         // X, UTF_16의 경우는 ASCII 2byte 처리로 실패
    }
